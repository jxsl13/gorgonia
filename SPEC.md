...
- V36: vendoring-via-`replace` (C12) must NOT change a vendored dep's PUBLIC interface that this module's CORE (always-compiled) code depends on: `replace` is ignored by downstream consumers, so a stock dep without the change breaks their build. Asm-only mods (vecf32/64) degrade gracefully (consumer loses the speedup, still builds); interface changes (tensor Pointer()) do NOT — those need a real published fork or no change at all (B11). The unsafeptr fix stays consumer-safe via cuda-gating (stock tensor).
...
T31|x|REVERTED (B11): vendoring tensor + Memory.Pointer() broke downstream consumers (replace is main-module-only -> stock tensor has no Pointer()). unsafeptr stays fixed consumer-safe via cuda-gating. A real fix needs a published tensor fork; not worth it|B11,V36,C12,B8
...
B11|2026-06-13|T31 vendored tensor via go.mod `replace` AND added Pointer() to the tensor.Memory INTERFACE; makeScalarFromMem (core, always compiled) called mem.Pointer(). But `replace` is main-module-only — a downstream consumer of github.com/jxsl13/gorgonia gets STOCK gorgonia.org/tensor (no Pointer()) -> 'mem.Pointer undefined' build break|revert T31: un-vendor tensor, drop replace, restore consumer-safe makeScalarFromMem (cuda-gated unsafe.Pointer(mem.Uintptr()) with stock tensor); V36
```
