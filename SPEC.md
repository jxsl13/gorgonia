...
B8|2026-06-13|PR CI darwin Vet step hard-failed on pre-existing legacy: op_tensor.go:438 unreachable dead code after return + 8 unsafeptr 'possible misuse' in values.go/values_utils.go (deliberate unsafe.Pointer(mem.Uintptr()) device-memory pattern, vet false positive)|removed dead line; vet step + Makefile use go vet -unsafeptr=false
```
